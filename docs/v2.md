...
````bash
curl -X GET "https://eos.hyperion.eosrio.io/v2/history/get_actions?noBinary=true"
````

checkLib - perform lib test. Default is false, meaning the query will be faster since it won't wait for the get_info to happen.

If you wish to see the lib value on the results you must request explicitly.
````bash
curl -X GET "https://eos.hyperion.eosrio.io/v2/history/get_actions?checkLib=true"
````

The simple mode will add the irreversible field (true/false) on each actions

````bash
curl -X GET "https://eos.hyperion.eosrio.io/v2/history/get_actions?checkLib=true&simple=true"
```` 
...
##### Example
```
curl -X GET "https://wax.eosrio.io/v2/state/get_links?account=eosriobrazil"
```
