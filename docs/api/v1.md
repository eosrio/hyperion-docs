...
| sort    | string  | sort direction <br>Enum: [ desc, asc, 1, -1 ]<br>`asc` requires a bound — see note | 
| after   | string($date-time)   |  filter after specified date (ISO8601) or block number
| before  | string($date-time)   |  filter before specified date (ISO8601) or block number
| parent  | integer <br>minimum: 0</br>  |  filter by parent global sequence

!!! warning "`sort=asc` requires a bounded query"
    As with the [v2 endpoint](v2.md#v2historyget_actions), an ascending sort is **rejected with HTTP 400**
    unless the request carries an `after` and/or `before` bound (an ISO8601 date or a block number).
    An ISO date `after` must fall within the last `max_asc_window_days` days (default `90`); block-number
    bounds are exempt, so use them to read older ranges. Self-hosted operators can disable the guard with
    `api.require_bounded_asc: false` *(Hyperion ≥ 4.1.0)*. `sort=desc` (the default) is unaffected.
