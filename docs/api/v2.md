...
| sort | query | Sort direction ('desc', 'asc', '1' or '-1'). `asc` requires a bounded query — see the note below | No | string |
...
!!! warning "`sort=asc` requires a bounded query"
    The action index is physically ordered for descending (newest-first) reads. An ascending sort
    without a bound forces Elasticsearch to scan the index in reverse across every shard, which is
    expensive on large chains. To prevent this, `sort=asc` (or `sort=1`) is **rejected with HTTP 400**
    unless the request also carries at least one bound:

    - `after` and/or `before` — an ISO8601 date **or** a block number. An ISO date `after` must fall
      within the last `max_asc_window_days` days (default `90`); block-number bounds are exempt from
      that recency window, so use them to read older ranges.
    - `global_sequence=<from>-<to>` or `block_num=<from>-<to>` *(Hyperion ≥ 4.1.0)* — a range, or a
      single positive value. Because `global_sequence` is the default sort field, such a range bounds
      the scan directly.

    ```
    # rejected — unbounded ascending scan
    /v2/history/get_actions?account=myaccount&sort=asc

    # ok — bounded by a recent date
    /v2/history/get_actions?account=myaccount&sort=asc&after=2026-05-01T00:00:00Z

    # ok — bounded by a block range (any age)
    /v2/history/get_actions?account=myaccount&sort=asc&after=425000000&before=425100000

    # ok (Hyperion ≥ 4.1.0) — bounded by a global_sequence range
    /v2/history/get_actions?account=myaccount&sort=asc&global_sequence=1000000-2000000
    ```

    `sort=desc` (the default) is unaffected.

    **Self-hosted operators** can disable this guard entirely by setting `api.require_bounded_asc: false`
    in the chain config *(Hyperion ≥ 4.1.0)* — this removes both the bound requirement and the
    `max_asc_window_days` recency window, accepting the performance cost of unbounded ascending scans.
